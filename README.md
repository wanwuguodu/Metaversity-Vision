...
**当前版本：v0.10，内容层次与设计质量。** 已与 Metaversity 应用统一阿里巴巴普惠体 3.0、文字层级、必填红星、默认收起的筛选浮层、起止范围交互、紧凑上下导航、手机字号密度、整框点击、圆角下拉及图片地图手势；后续所有相关列表和大图沿用本设计。大图操作统一为手机双指缩放、单指拖动，电脑滚轮缩放、右键拖动；不显示放大、缩小、重置／适配按钮、比例或缩放工具条；图集用前后按钮翻页。这里展示的是可复用规则与组件，业务数据均为示例，没有连接学校系统。
