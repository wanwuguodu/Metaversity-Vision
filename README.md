...
**当前版本：v0.13，首页与社团紧凑版式。** 参数与说明直接排版，字号表和办理指南并排展示，业务样例与设计讲解分别呈现。字体、颜色及交互规则与 Metaversity 应用一致；所有数据和业务操作均为演示，没有连接学校系统。
...
- **紧凑目录**：首页使用短入口名称与 68px 常用入口；社团示例采用文字下划线栏目、整行分隔列表和单行简介，点击后查看完整介绍。手机字号及至少 44px 点击区域保持原规范。

- **内容优先**：业务页直接展示内容，去掉演示、测试、实现说明及内部元数据；保留必要业务规则和状态。首页从全部服务进入完整目录，预约采用按校区分组的轻量条目。

- **搜索直达**：顶部只保留紧凑搜索框，搜索功能入口与功能大分类；当前统一南航，个人页显示学校名称，业务页不再重复显示演示环境横幅。
...
- **大图手势**：电脑滚轮缩放，按住滚轮（中键）或右键拖动；手机双指缩放、单指拖动。左键保留入口点选，画布外保持浏览器行为。
- **内容决定布局**：业务列表保持一致的阅读顺序，校园图文按主次分配空间；不凑数排卡片，不添加无意义装饰，文案与数据准确可信。
- **动效解释变化**：以短暂反馈确认操作，状态即时生效；减少动态效果时保留完整结果，不依赖动画完成任务。
...
页面以章节目录查阅：色值使用紧凑色板，字号层级配合校园卡办理指南，间距与圆角直接展示参数；实际组件保留所需表面和背景。规范说明置于样例外，导航、表单和结果状态使用具体的校园任务文案。

可实际操作功能与分类搜索、学校切换示例、活动搜索、筛选浮层、条件计数、日期范围中文滚轮与取消回退，以及表单校验、轮播切换、首页与个人切换、详情返回、重试反馈、确认对话框、底部面板和提示消息。图文、办事入口与修读摘要组合展示内容主次；收藏示例演示即时状态和短暂图标反馈。大字与减少动态效果开关便于对比体验。视频区域仅展示封面入口，深色模式尚未定稿。
...
发布前按规范第 10.1 节检查任务主次、内容布局、文案素材、移动端与桌面布局、键盘操作、表单与弹层状态、文档链接及示例数据说明；同时核对减少动态效果下的即时反馈。筛选重点核对展开不推挤列表、独立滚动和焦点恢复；范围重点核对自动切换、日期上下限和取消回退。浏览器验证记录见 `docs/development/`，不将浏览器视口模拟等同真实设备验收。
