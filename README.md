...
**当前版本：v0.4，筛选浮层与时间范围统一。** 已与 Metaversity 应用统一阿里巴巴普惠体 3.0、文字层级、必填红星、默认收起的筛选浮层和起止范围交互；后续所有相关列表沿用本设计。这里展示的是可复用规则与组件，业务数据均为示例，没有连接学校系统。
...
| Web 紧凑筛选 | 普通控件至少 44px；正文 16/24，短选项并排、窄屏自然换行 |
...
可实际操作活动搜索、筛选浮层、条件计数、日期范围中文滚轮与取消回退，以及表单校验、轮播切换、首页与个人切换、详情返回、重试反馈、确认对话框、底部面板和提示消息。大字与减少动态效果开关便于对比体验。视频区域仅展示封面入口，深色模式尚未定稿。
